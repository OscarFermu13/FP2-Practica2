...
/* FUNCION numElementos */
/* Extrae la información del fichero de texto "numeros.txt" y los coloca en un array.*/
/* Cuenta el numero de elementos que tiene el fichero para pasarselo posteriormente
a la funcion cercaDicotomica.*/
int numElementos(int numeros[100]){
    char string[4];
    int j=-1;
    char *devf;

    FILE *ifp;
    ifp=fopen("numeros.txt","r");
    int k = 0;
 
    do
       {
        j++;
        devf = fgets(string,4,ifp);
        numeros[k] = atoi(string);
        k++;
       } while (devf!=NULL);
...
/* Retorna un numero */
int cercaDicotomica(int numeros[100], int l, int r, int x)
{
      if (r >= l) { 
        int mitad = l + (r - l) / 2; 
        if (numeros[mitad] == x) 
        ...
        if (numeros[mitad] > x) 
            return cercaDicotomica(numeros, l, mitad - 1, x); 
  

        return cercaDicotomica(numeros, mitad + 1, r, x); 
    } 
    return -1; 
}
